...
public class DealManager {
    ...
    public ArrayList<Deal> updateCacheList() {
        ...
        try {
            ...
            dealList = query.find();
            deals = new ArrayList<Deal>(dealList);
    ...
    public void updateSeatOffer(int num, String refId) {
        final int seat = num;
        ParseQuery<Deal> query = ParseQuery.getQuery(PARSE_NAME);
        query.whereEqualTo("objectId", refId);
        query.findInBackground(new FindCallback<Deal>() {

            public void done(List<Deal> list, ParseException e) {
                if (e == null) {
                    Deal deal = list.get(0);
                    deal.put("seatToOffer", seat);
